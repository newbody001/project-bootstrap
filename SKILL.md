...
③ **功能注入模式**（核心中的核心） — 告诉 Agent "加 X 功能"时按什么步骤、改哪些文件。至少包含 2 个模式（如"新增 API 端点"和"新增数据模型"），每种模式给出明确的步骤清单表和参考位置。

④ **关键注意事项** — 列出 2~5 条容易踩坑的点（配置约定、硬编码位置、特殊文件结构等）

**选填区域（有则填）：**

- **架构依赖关系** — 如果项目有明确分层（路由层/业务层/数据层/视图层），**强烈建议填写**。标注"换框架/换数据库"时哪些文件受影响、哪些安全。
